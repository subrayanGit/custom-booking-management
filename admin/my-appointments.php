...
if (isset($_SESSION['ct_login_user_id']))
{
    ...
    while ($dd = mysqli_fetch_array($details)) { ?>                    
		...
		echo  $current_times = date('Y-m-d H:i:s', $currDateTime_withTZ); echo "///";
        echo $bt = date('Y-m-d H:i:s', strtotime($current_times));
        if ($bt < $td)
        ...
        else
        {
            if ($dd['booking_status'] == 'A' || $dd['booking_status'] == 'C' || $dd['booking_status'] == 'RS')
            {
                ...
                echo $remain_times = $booking_start_datetime - $current_time;
